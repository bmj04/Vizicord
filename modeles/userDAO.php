<?php
    ...
    class userDAO implements InterfaceDAO {
        ...
        public function createPost($userId, $title, $description, $category, $status, $file, $pdo) {
            $destination_path = dirname(__DIR__) . "\\uploads\\" . $file['name'];
        ...
        public function verifyAdmin($userId, $pdo){
                $stmt = $pdo->prepare("SELECT admin_token FROM users WHERE id = :userId");
                $stmt->bindParam(':userId', $userId);
                $stmt->execute();
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($result['admin_token'] !== null){
                    return true;
                }
                else{
                    return false;
        ...
        public function toPin($postId, $pdo, $userId){
            $sql = "UPDATE users set pinned = :postId WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userId);
            $stmt->bindparam(":postId", $postId);
            $stmt->execute();
        ...
        public function getPin($userId, $pdo){
            $stmt = $pdo->prepare("SELECT pinned FROM users WHERE id = :userId");
            $stmt->bindParam(':userId', $userId);
            $stmt->execute();
            $result =  $stmt->fetch(PDO::FETCH_ASSOC);
            return $result;
